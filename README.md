...
Adds a seventh quality tier to everything in the game that has a quality: 43 melee and ranged
weapons (junk sledge, turret and drone included), 16 tools, 66 armour pieces, 12 clothing pieces and
the 35 item mods. New weapon *tiers*, meaning guns that do not exist in vanilla, are a separate mod;
this one only extends what is already there.

| | |
|---|---|
| **Extra mod slot** | tier 7 grants one more than tier 6 |
| **Stats** | every tier-scaled value continues at the same per-tier step vanilla uses from 1 to 6 |
| **Where it comes from** | crafted, or looted five times less often than tier 6 - no trader stocks it |
| **Cost** | six Legendary Parts, where tier 6 asks for one |
| **Junk items** | free tier 7, but no extra mod slot - see below |
| **Item mods** | loot only, because vanilla never crafts a mod above quality 1 |
| **Colour** | gold, one step past the legendary purple - and quality is shown as a number, not a word |
...
The config is generated from the vanilla XML rather than hand written - 172 items times several
tier ladders is not maintainable by hand, and regenerating against a patched game shows at once
what The Fun Pimps moved:
...
`verify-t7-patches.ps1` replays every generated patch against the vanilla config the way
`XmlPatcher` would and reports any that match nothing - which is how a wrong predicate would
otherwise fail: silently, with the tier simply never appearing. It also checks that no effect's
level range stops below a raised skill cap, since anything that does quietly switches itself off up
there, and that the two lists the generator and the DLL both keep have not drifted apart.
